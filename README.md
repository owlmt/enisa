...
> **Version note.** Section and note numbers below are keyed to the **adopted v2.0 (May 2025)**, the version currently applicable for certification. They are cited at section/note granularity rather than by page number, since pagination varies across renders. A **v3.0 draft is in public review until end of July 2026**; it renumbers most notes, relabels "legacy → admissible", and adds mechanisms absent from v2.0 (Argon2-id with a 128-bit salt, SP 800-227 KEM combiners, EdDSA, SHAKE/cSHAKE XOFs). The appendix maps the v3.0-draft note numbers to v2.0 so this list stays usable against either. **This list targets v2.0 (the applicable version); the v3.0 draft should be checked against it before relying on a specific note number.**
...
> *Draft-only:* the v3.0 draft adds Argon2-id with an agreed parameter set that also fixes a **128-bit salt** (Note 27-Argon Parameters). Not present in v2.0.
...
> *Draft-only:* the v3.0 draft adds EdDSA with **Note 51-DeterministicSignatures** — its per-signature value is derived **pseudo-randomly from secret key + message** (no RNG needed) but is fault-attack susceptible. Not present in v2.0.
...
## Appendix — Note-number map (v3.0 draft ↔ adopted v2.0)

| Topic | v3.0 draft | **Adopted v2.0** |
|-------|------------------|--------------------|
